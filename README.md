...
由于子智能体应当独立于主智能体运行，而同样拥有反馈机制，所以该功能并没有内置进突出助理本体。

提示主智能体读取子智能体技能，随后主智能体将根据任务需求设计任务规划，生成配置文件。

生成后启动SynapseAgent_SubAgent.py或者start_subagent.bat，启动后程序将提示输入配置文件地址，输入配置文件地址或者直接拖进去。随后，程序将根据任务数量启动子智能体，各智能体将并行行动，直到完成各自任务。

子智能体完成任务后会提交任务简报，写入配置文件。主智能体的后续任务可能需要参考修改后的配置文件来了解执行情况。

**并不建议使用该功能。突触助理的设计定位为协作助手，而并非任务规划者，也并不应当有自主决策的权力。子智能体仅为技术验证和特殊需要情况下使用。**
